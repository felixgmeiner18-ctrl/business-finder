...
# Letter tracking landers (e.g. /VB02) — see GET /{code} below
PUBLIC_PATH_PATTERN = re.compile(r"^/VB\d{2}$")


@app.middleware("http")
async def basic_auth(request: Request, call_next):
    path = request.url.path
    if path in PUBLIC_PATHS or PUBLIC_PATH_PATTERN.match(path):
...
async def submit_contact(payload: ContactPayload):
    # Lander-form submissions don't ask for email (per letter promise:
    # "nur Firma, Telefonnummer und zwei Sätze"). So email is optional;
    # name + (phone OR message) is enough to identify the lead.
    ...
    if not message:
        raise HTTPException(400, "Nachricht ist erforderlich")
